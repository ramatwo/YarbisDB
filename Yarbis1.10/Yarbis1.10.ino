...
// Cuánto puede cambiar la velocidad por ciclo (10ms).
// 15 = llega de 0 a 255 en ~17 ciclos = ~170ms
#define RAMP_STEP 15
...
// velocidades actuales con ramping (lo que realmente se manda al motor)
int rampedVelA = 0;
int rampedVelB = 0;
...
// Acerca 'current' hacia 'target' de a RAMP_STEP por llamada
...
void processGamepad(ControllerPtr ctl) {
    ...
    int targetA = velY + velX;
    int targetB = velY - velX;
    ...
    targetA = constrain(targetA, -255, 255);
    targetB = constrain(targetB, -255, 255);

    // ramping: acercar velocidad actual al objetivo de a RAMP_STEP
    rampedVelA = rampToward(rampedVelA, targetA);
    rampedVelB = rampToward(rampedVelB, targetB);

    setMotorBTS(bts1, rampedVelA);
    setMotorBTS(bts2, rampedVelB);
}
